...
-- (ibtotal_points/no_of_matches)*100 as win_percentage 
...
select  ibp.bidder_id , ibp.no_of_bids , ibp.no_of_matches , (count(ibd.bid_status)/ibp.no_of_matches)*100 as percent ,ibd.bid_status from ipl_bidder_points as ibp
 join ipl_bidding_details as ibd on ibp.bidder_id=ibd.bidder_id  where ibd.bid_status = 'won' or 'loss' group by ibp.bidder_id,ibd.bid_status order by percent desc ;

-- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database
select * from ipl_stadium;
select * from ipl_match_schedule;

select  count(ims.stadium_id ) as number_of_matches_conducted_at_each_stadium  , ist.stadium_name , ist.city  
 from ipl_stadium as ist join ipl_match_schedule as ims on ist.stadium_id=ims.stadium_id group by ims.stadium_id ;
 
 -- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
 select * from ipl_match;
 select * from ipl_match_schedule;
 select * from ipl_stadium;
 -- count(im.match_winner)

select if(im.toss_winner=1,im.team_id1,im.team_id2) as toss_winner , if(im.match_winner=1,im.team_id1,im.team_id2) as match_winner,ist.stadium_name ,
(count(toss_winner)/count(match_winner))*100 as percentage
from ipl_match as im join ipl_match_schedule as ims
on im.match_id = ims.match_id
join ipl_stadium as ist on ims.stadium_id=ist.stadium_id 
where toss_winner = match_winner group by im.match_id order by percentage desc;
 ...
 select count(idp.no_of_bids) as number_of_bid, ibd.bid_team , ipl.team_name from ipl_bidder_points as idp join ipl_bidding_details as ibd on idp.bidder_id=ibd.bidder_id 
 join ipl_team as ipl on ibd.bid_team=ipl.team_id group by ipl.team_name ;
 
 
 -- 5.	Show the team id who won the match as per the win details.
 select * from ipl_match;
  select * from ipl_team;
 select it.team_id ,it.team_name,if(im.match_winner=1,im.team_id1,im.team_id2) as matchs,im.WIN_DETAILS from ipl_team as it
 join ipl_match as im  having it.team_id=matchs;
 ...
 select its.tournmt_id,its.matches_played,its.matches_won,its.matches_lost ,it.team_name from ipl_team_standings as its join  ipl_team as it on its.TEAM_ID=it.TEAM_ID
 group by its.tournmt_id,it.team_name;
 
 
 -- 7.	Display the bowlers for Mumbai Indians team.
 select * from ipl_team_players where PLAYER_ROLE like '%all-rounder%';
 select * from ipl_player;
 select ip.player_id,ip.player_name,itp.player_role ,itp.remarks from ipl_player as ip join ipl_team_players as
 itp on ip.player_id=itp.player_id where itp.player_role ='bowler' and itp.remarks like '%mi%' ;
 
 -- 8.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.
select count(player_role) as cnt , player_role,REMARKS from ipl_team_players  where PLAYER_ROLE like '%all-rounder%' group by REMARKS  having cnt > 4
 order by cnt desc ;
